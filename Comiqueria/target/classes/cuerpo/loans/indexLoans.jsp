  ...
  <div class="busqueda">
    <div class="buscar">
      <input type="text" id="search" name="txtBuqueda" placeholder="Ingrese busqueda...">
       <a class ="action" href="/Comiqueria/insertar_Prestamos"><button class="vista cancelacion" type="button" name="alta">Nuevo</button></a>
      <button class="vista exit" type="button" name="salir" onclick="">Atras</button>
    </div>
    </div>
    <div class="datagrid"><table  id="table">
      <thead><tr  class="bind"><th>Comic</th><th>Person</th><th>Date</th><th></th></tr></thead>
    ...
    <c:choose>
	 ...
	 <c:choose>
      <c:when test="${i mod 2 ne 0}">
          <tr class='alt'>
        <td>  <c:out value="${Listado.get(i).getComic().getNameComic()}"></c:out></td>
        <td><c:out value="${Listado.get(i).getPerson().getNamePerson()}"></c:out></td>
        <td><c:out value="${Listado.get(i).getDate()}"></c:out></td>
        <td>
         <div class="wwith">   <a href="/Comiqueria/actualizar_Prestamos?id=${Listado.get(i).getIdLoan()}"> <img  src="/Comiqueria/imgs/editar.png"></a> </div>
        <div  class="wwith">    <a href="/Comiqueria/eliminar_Prestamos?id=${Listado.get(i).getIdLoan()}"><img src="/Comiqueria/imgs/eliminar.png"></a>   </div>
         </td></tr>
      </c:when>
      <c:otherwise>
      <tr>
