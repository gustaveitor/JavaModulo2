...
//	private final String SQL_UPDATE = "UPDATE users SET nameUser = ? , passwordUser=? WHERE idUser = ? ";
	...
	private final String SQL_GETALLR = "select Users_idUser, Roles_idRole,r.nameRole from comics.users_roles inner join comics.roles as r on Roles_idRole=r.idRole where Users_idUser=?";
	...
	public boolean add(User c) throws Exception  {
		try {
			...
			throw exceptionConverter.getException("Problemas en la base de datos USUARIOS", e) ;
	...
	@Override
	public boolean update(User c) throws Exception{
/*		PreparedStatement ps;
		...
		if (ps.executeUpdate() > 0) {
			...
			//NO ESTA HECHO DEBIDO A QUE NO HACEMOS ABMS DE ROLES NI USUARIOS
			return true;
		} catch (SQLException e) {
			new exceptionConverter();
			throw exceptionConverter.getException("Problemas en la base de datos USUARIOS", e) ;
